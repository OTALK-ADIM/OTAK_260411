...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const initRadar = async () => {
      ...
      // 💡 핵심 해결책: 감지기 이름 뒤에 겹치지 않게 시간을 붙여서 충돌을 원천 차단합니다!
    ...
    <div className="w-full flex flex-col font-mono relative">
      ...
      {!user ? (
        ...
        <div className="nuke-menu-container">
          ...
          <div className="w-full mb-8 relative" ref={logRef}>
            <div 
              ...
              className={`w-full border-2 p-4 cursor-pointer flex justify-between items-center transition-all ${
                unreadCount > 0 ? "border-red-500 bg-red-950/20 text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.3)]" : "border-green-900 bg-black text-green-900"
              }`}
            >
                    ...
                    notifs.map(n => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-[10px] font-bold uppercase">{n.type === 'COMMENT' ? "TYPE: COMMENT" : "TYPE: DM_REQUEST"}</span>
                          <span className="text-[10px] opacity-70">{new Date(n.created_at).toLocaleString()}</span>
                        </div>
                        <div className="text-sm md:text-base font-bold">&gt; {n.from_nickname}님이 {n.type === 'COMMENT' ? "댓글을 남겼습니다." : "비밀 통신을 요청했습니다."}</div>
                      </div>
                    ))
                  )}
                </div>
          ...
          <div onClick={() => setLocation("/rules")} className="nuke-menu-item transition-none group"><span className="nuke-menu-icon">▶</span> 0. NERD_PROTOCOL (규칙)</div>
          <div onClick={() => setLocation("/feed")} className="nuke-menu-item transition-none group"><span className="nuke-menu-icon">▶</span> 1. 활동 모집 피드</div>
          <div onClick={() => setLocation("/chat-list")} className="nuke-menu-item transition-none group"><span className="nuke-menu-icon">▶</span> 2. 대화함</div>
          <div onClick={() => setLocation("/profile")} className="nuke-menu-item transition-none group"><span className="nuke-menu-icon">▶</span> 3. 나의 데이터 (프로필)</div>
        </div>
      )}
    </div>
  );
